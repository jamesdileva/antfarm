...
| 2026-08-25 | S16 | Colony lifecycle: archive command (project+db+config snapshot to archives/<ts>, CLI + POST /api/lab/archive), GUI Archive/Reset buttons with confirm guards (reset refuses while colony running), reset auto-archives by default (--no-archive opt-out); shell single-instance lock after double-launch EADDRINUSE confusion; e2e isolated via --home temp dir after smoke run polluted real lab | 105/105 pass - build green - e2e green | 9c39f6e |
| 2026-08-25 | S16.1 | Goal presets per mode: Directed (docs-driven, incl. generate-docs-if-missing clause) and Constrained (local-only) fill editable textarea + set config mode on Set goal; Clear goal = autonomous (initLab clearGoal deletes PROJECT_GOAL.md); two template-literal escape bugs found via CDP DOM inspection (dollar-brace in comment, apostrophe in preset text) - blank-page class eliminated | 106/106 pass - build green - e2e green | d2397ba |
...
| 2026-09-23 | S16.5 | Board-protocol fix: prompt states TASK-mail creates/taskMoves moves, string taskId coercion, data.info guard, phantom-id teaching WARNING, same-state no-op; adversarial audit.md (4 critical/4 high/8 med); colony notified via mail #79/#80 | 119/119 pass - build green - bundle restaged | 7ccf9bc |
| 2026-09-24 | S16.6 | Audit remediation: token auth on all serve routes, strict bodies + fail-closed reset, settings value validation, prompt provenance, colony lockfile, HOME alias, caps/sanitize/CSP; e2e PASSED on packaged exe; M8 deferred (baby-agent corpus) | d87024e |
